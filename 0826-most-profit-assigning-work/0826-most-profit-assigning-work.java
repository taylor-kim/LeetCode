class Solution {
    public int maxProfitAssignment(int[] difficulty, int[] profit, int[] worker) {
        return official_bs_profit_base(difficulty, profit, worker);
    }

    public int official_bs_profit_base(int[] difficulty, int[] profit, int[] worker) {
        int n = profit.length;
        ...
        for (int i = 0; i < n - 1; i ++) {
            datas[i + 1][1] = Math.min(datas[i][1], datas[i + 1][1]);
        }
