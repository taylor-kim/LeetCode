        ...
        for (int[] direction : DIRECTIONS) {
            ...
            if (isValidLandCell(grid, newRow, newCol)) {
                if (discoveryTime[newRow][newCol] == -1) {
                    ...
                    // if (lowestReachable[row][col] > lowestReachable[newRow][newCol]) {
                    //     System.out.println(String.format("what case - row:%d, col:%d, newRow:%d, newCol:%d, rc:%d, nrnc:%d", row, col, newRow, newCol, lowestReachable[row][col], lowestReachable[newRow][newCol]));
                    // }
